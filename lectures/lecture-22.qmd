...
:::: {.columns}
::: {.column width="60%"}
_ An *analog* signal is something that changes smoothly over time and space.  Mathematically, analog signals are represented by *functions* of *real numbers*.
- A *digital* signal comes in countable pieces.  Mathematically, digital signals are represented by *sequences* of *integers*.
...
- The *sampling rate* is the number of time steps in one second (e.g. 40000 samples per second)
- The *bit depth* gives the number of steps in amplitude
