...
We're going to keep a very basic definition of pitch and define it as the way we distiguish keys on a piano.
...
In Western music, notes separated by an octave are given the same pitch name (letter).  When played together, human hearing will interpret these as the same pitch.
...
$$
In words, the ratio is the twelfth root of 2.  We can also write this as $2^{1/12}$, 2 to the 1/12th power, which is easier to do algebra with.
...
TODO picture from W&W

## Cents
Because multiplying and dividing frequencies is inconvenient, we often characterize pitch in terms of a measure called *cents* that we can add or subtract.  Cents live on a logarithmic scale, such that adding 1200 cents doubles the frequency.
Adding or subtracting 100 cents moves us by one semitone (one piano key) up or down on the equal tempered scale.

If we have two frequencies, their difference in cents is given by
$$
\mathrm{cents} = 1200 \log_2 (f_1/f_2).
$$
...
$$
\mathrm{cents} = \frac{1200}{\log 2} \log (f_1/f_2)
...
Going the other way, if we know *f*~1~ want to change by a number of cents, the new frequency *f*~2~ is
$$
