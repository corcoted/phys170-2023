...
One detail the book leaves out in Chapter 9 is the link between changes in air pressure and the sound intensity. We need this information to link to the various measures of loudness in this chapter.
...
Recall from last time that the minimum sound intensity a typical adult human can hear is *I*~0~=10^-1^ W/m^2^. This should be about what we get from the minimum pressure (*p*~0~=2×10^-5^ Pa) and the impedance of air (*z*~0~=413 Pa s/m).
...
::: columns
::: {.column width="50%"}
Another measure of subjective loudness is the sone (rhymes with "bone"). The sone tries to capture the idea of what it means for one sound to be "twice as loud" as another sound. The basic relationship is that multiplying the sound intensity by 10 (equivalently, adding 10 dB to the sound level or adding 10 phons) creates a sound that seems 2 times as loud to an average adult human.
...
::: {.column width="50%"}
| Dynamic Level | Sones | Phons | Sound level (dB) @ 1kHz | Sound intensity (W/m2) @ 1 kHz |
|---------------|-------|-------|-------------------------|--------------------------------|
| fff           | 64    | 100   | 100                     | 10^-2^                         |
| ff            | 32    | 90    | 90                      | 10^-3^                         |
| f             | 16    | 80    | 80                      | 10^-4^                         |
